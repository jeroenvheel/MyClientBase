...
class MY_Parser extends CI_Parser {
	...
	public function parser_callback($plugin, $attributes, $content)
	{
		...
		if ( ! $attributes)
			{
				return '** Parser:Missing  var=" ...... " **';
			}
	
		$this->_ci->load->model('invoices/mdl_invoices');
	
		$params = array(
			'where'	=>	array(
				'mcb_invoices.invoice_id'	=>	$attributes['var']
			),
		...
		$invoice = $this->_ci->mdl_invoices->get($params);

		$result = $plugin($invoice);
